...
**Branch**: `001-idx-prediction-product` | **Date**: 2026-05-29 | **Spec**: [spec.md](spec.md)
...
prediction product. Users choose a curated model, review concise evidence,
select supported IDX stocks, and quickly understand a next-market-session model
signal with confidence, context, limitations, timestamps, traceability, and a
clear educational/research disclaimer.

The next implementation increment is a prediction-first UX revision of the
current working skeleton. The technical architecture remains separated:
Streamlit owns presentation, FastAPI owns service orchestration and contracts,
Python ML modules own model/feature/prediction/validation logic, SQLite owns
lightweight catalogue/evidence/log metadata, and MLflow owns model tracking and
artifact metadata. The plan preserves existing safety gates while changing the
main screen so the prediction, confidence, key context, and limitations are the
primary visible content after a valid model and ticker are selected.

Local development may keep using mock catalogue/evidence data and dummy
prediction outputs to exercise the skeleton. The public demo must use real
approved models only. If no real approved model and approved non-sensitive data
source are available, public prediction is blocked and the app may only run as a
clearly labelled degraded demo.
...
contract, deployment smoke tests, and UI-facing copy/presentation rules. FastAPI
tests use TestClient. Streamlit is tested through API-client/service boundaries,
copy helpers, presentation component behavior where practical, and manual
...
within a documented model-specific budget for real models. A non-expert user can
identify the selected ticker, model signal, confidence category, and key
limitation within 10 seconds after selecting a supported ticker.
...
chronological integrity for all evidence and prediction workflows, prediction
outputs/logs must include data-as-of and feature-generation timestamps,
registry/catalogue conflicts fail closed, required evidence must be loaded and
valid before prediction, public-demo predictions must come from real approved
models only, and the main UI must not bury prediction output beneath dense
methodology or raw traceability text.
...
complex modelling until the core product, safety, traceability, and
prediction-first user journey work clearly.
...
- [x] No Financial Advice: UI copy, API schemas, prediction outputs, evidence
      ...
      The prediction-first screen still describes output as a model signal, not
      advice.
...
- [x] Evidence Before Prediction: The Streamlit flow and API expose required
      evidence before or alongside prediction interpretation. The revised UX may
      use a concise evidence summary in the primary journey, but prediction
      remains blocked when required evidence is missing, stale, unavailable, or
      not loaded.
- [x] Honest Uncertainty: Prediction responses include Low/Medium/High
      ...
      evidence caveats. Weak or data-limited evidence is surfaced in calm
      product language; missing required evidence blocks approval or prediction.
...
- [x] Reproducibility and Traceability: Prediction logs and outputs include
      ...
      success/failure status. Traceability is visible but secondary in the main
      UI.
- [x] Separation of Concerns: Streamlit handles presentation and copy only;
      FastAPI validates and orchestrates requests; shared modules own ML,
      validation, backtesting, paper trading, storage, registry, evidence, and
      governance logic. Route handlers do not contain core ML logic, and UX
      revisions do not move prediction or feature logic into UI code.
- [x] Testability: Planned tests cover model filtering, registry/catalogue
      ...
      restrictions, data governance, degraded public-demo release gates, error
      handling, prediction-first presentation order, copy clarity, and removal
      of awkward action labels.
- [x] User Clarity: The main journey becomes choose model, choose ticker,
      immediately understand prediction, then inspect supporting evidence,
      limitations, and traceability. Technical details are available but
      visually secondary.
- [x] Data Governance: Secrets, credentials, private datasets, proprietary
      ...
      public predictions.
- [x] Simplicity Before Complexity: The next increment is a focused UI/UX and
      copy revision of the existing skeleton. No new modelling complexity,
      high-availability infrastructure, or full paper-trading implementation is
      introduced before the core product flow is understandable.
- [x] Academic and Portfolio Integrity: Methodology, limitations, evidence
      ...
      approval criteria are documented and traceable without overstating model
      capability.
...
- `app_streamlit/`: Customer-facing screens, concise disclaimers, model/ticker
  selectors, prediction-first layout, confidence explanation, "why this signal
  appeared" section, readable limitations, secondary evidence and traceability
  details, unavailable states, and API client code. It must not contain
  training, feature-engineering, backtesting, or prediction model logic.
...
- `docs/`: Architecture, methodology, limitations, model approval process,
  evaluation notes, deployment documentation, and reviewer validation notes.
...
4. Evidence retrieval returns historical metrics, evaluation period, supported
   ...
   historical-performance caveat. Streamlit presents a concise evidence summary
   before or alongside prediction interpretation and keeps detailed methodology
   secondary.
...
9. Streamlit renders the response as a prediction-first view: selected ticker,
   model signal, confidence, why the signal appeared, and key limitations appear
   before dense methodology or raw traceability details. The UI uses plain
   product language, avoids cell/tag styling for limitations, and uses a
   lightweight action label such as "Predict" only if automatic prediction is
   not practical.
...
│   ├── evidence_panel.py
│   ├── framing.py
│   ├── paper_trading_evidence.py
│   ├── prediction_result.py
│   └── unavailable_state.py
├── clients/
│   └── api.py
└── copy/
    └── disclaimers.py
...
for the first version. The UX revision stays inside `app_streamlit/` and copy
tests unless backend contracts are missing required data.

## Implementation Phases

### Phase 1: Product Skeleton Completed

- Project structure exists.
- Streamlit, FastAPI, SQLite, local registry helpers, dummy prediction, and
  safety tests exist.
- Local-only mock model catalogue, mock evidence catalogue, and dummy
  prediction output exercise the full flow.
- Local mock/dummy outputs are labelled as development-only and are blocked
  from public-demo eligibility.

### Phase 2: Prediction-First UX Revision

- Rework the Streamlit main flow so selecting a valid model and supported ticker
  makes the prediction area the primary visible content.
- Prefer automatic prediction after valid selections and passing evidence/data
  gates. If manual action remains necessary, use the label "Predict".
- Rewrite primary labels, warnings, limitations, evidence summaries, and error
  messages in plain product language for non-expert users.
- Replace raw metadata, isolated tags, or cell-like limitation styling with
  readable guidance sections.
- Group the main screen into prediction, confidence, why this signal appeared,
  key limitations, model evidence, and traceability details.
- Keep detailed evidence, methodology, and traceability secondary or
  expandable, while preserving evidence-before-prediction and reviewer access.
- Add focused tests for copy safety, presentation order, absence of awkward
  action labels, readable limitation display, and no-advice framing.

### Phase 3: Contracts And Storage Hardening

- Keep Pydantic request/response schemas and OpenAPI contract aligned with the
  prediction-first UI. Do not add contract fields only for decoration if the UI
  can derive readable sections from existing response fields.
- Maintain SQLite tables for model catalogue, evidence metadata, supported
  stocks, prediction logs, disclaimers, data availability, registry sync status,
  registry conflict logs, and unsupported-stock interest.
- Preserve validation for approved models, registry/catalogue consistency,
  supported tickers, required evidence availability/load status, feature
  timestamp validity, and prediction output shape.
- Ensure prediction logging and traceability metadata include data-as-of
  timestamp, feature-generation timestamp, and prediction timestamp.

### Phase 4: Tests And Safety Checks
...
- Test the revised customer-facing flow for prediction-first visibility,
  readable grouping, natural copy, and non-expert comprehension criteria.

### Phase 5: MLflow And Real Model Integration
...
- Remove dummy fallback from any public-demo path. Keep dummy prediction only
  for local development tests if still useful.

### Phase 6: Evidence And Paper-Trading Display
...
- Make weak, stale, missing, or data-limited evidence visible and actionable
  without overwhelming the main prediction view.
- Add plain-language limitation summaries for non-expert users.

### Phase 7: Public Demo Deployment
...
- Unit tests: domain rules, disclaimer copy, product copy wording, model
  approval decisions, fail-closed registry decisions, confidence labels, data
  availability, feature timestamp validation, unsupported stock handling, and
  presentation helpers for prediction-first sections.
...
- Integration tests: Streamlit client to API service flow, SQLite repositories,
  MLflow catalogue sync, conflict logging, prediction logging, evidence gating,
  and automatic or lightweight prediction request behavior when selections are
  valid.
...
- UI/product-safety checks: no "Request educational prediction" label, no
  forbidden financial-advice phrases, prediction appears before dense
  methodology, limitations are readable guidance rather than raw tags/cells,
  and traceability details remain secondary but available.
...
- Financial-advice wording risk: centralize disclaimer/copy constants, test
  forbidden phrases, and avoid buy/sell/recommendation language while keeping
  primary action labels natural.
- Text-heavy UX risk: make prediction output the primary visual element, use
  concise evidence summary in the main journey, and move dense methodology and
  traceability into secondary sections.
- Awkward copy risk: audit labels, warnings, evidence summaries, limitations,
  and unavailable states for plain product language before task completion.
...
- Over-engineering risk: keep the next increment focused on the Streamlit UX,
  copy, presentation tests, and existing service contracts before adding new
  model capability or infrastructure.
...
production infrastructure. The UX revision uses existing service contracts
where possible to avoid backend churn.
